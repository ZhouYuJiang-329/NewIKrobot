...
{    
    public class Position
    {
        ...
        public Position(double joint1, double joint2, double joint3, double joint4, double joint5, double joint6, Matrix4x4? Grip2Tool = null)
        {
            ...
            pose = new CartesianPosition(PathClass.FKNewRad(new double[] { joint1, joint2, joint3, joint4, joint5, joint6 }, Grip2Tool));
        }

        public Position(double[] joint, Matrix4x4? Grip2Tool = null)
        {
            joints = new JointPosition(joint);
            pose = new CartesianPosition(PathClass.FKNewRad(joint, Grip2Tool));
        }
        ...
        {//20240516  直接用继承  用基类进行统一（方式之一）    movel 重载   代码规范性
            ...
            //传入的是m
            endposition = PathClass.IK_New(endPose);
            double[] q = PathClass.SolveQ(endPose.X, endPose.Y, endPose.Z, endPose.t);


            double distance = Math.Sqrt(Math.Pow((endPose.X*1000 - startPos.pose.X), 2) + Math.Pow((endPose.Y *1000- startPos.pose.Y) , 2) + Math.Pow((endPose.Z * 1000  - startPos.pose.Z) , 2));
            ...
            for (int i = 1; i <= numPoints; i++)
            {
                ...
                temp.pose = CartesianPosition.Lerp(startPos.pose.mmTom(), endPose, t);
                ...
                temp = PathClass.IK_New(temp.pose);
                double[] qq = PathClass.SolveQ(temp.pose.X, temp.Pose.Y, temp.Pose.Z, temp.pose.t);
