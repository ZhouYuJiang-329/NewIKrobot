...
using static RobotLibraryAlgorithm.BasicAlgorithm;

namespace RobotLibraryAlgorithm
{
    public class Position:BasicAlgorithm
    {
        ...
        {//20240516  直接用继承  用基类进行统一（方式之一）    movel 重载   代码规范性
            ...
            base.InitialOrientInpParam(BasicAlgorithm.RxyzToRotMatrix(startPos.pose.RX, startPos.pose.RY, startPos.pose.RZ), BasicAlgorithm.RxyzToRotMatrix(endPose.RX, endPose.RY, endPose.RZ), Param);
        ...
        ****************************************************************************/
        public  Vector2 DisplacementInTool(double[] startAngle, double[] endAngle, Matrix4x4? Grip2Tool = null)
        {
            ...
            Position startPosition = new Position(base.AngleToRad(startAngle));
            Position endPosition = new Position(base.AngleToRad(endAngle));
